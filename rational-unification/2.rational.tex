...
We define the set of term subterms $\subterms : T^\infty \rightarrow 2^{T^\infty}$\inRocq{inf_subterm} in a conventional way inductively:
\begin{itemize}
    \item $t \in \subterms(t)$;
    \item $t \in \subterms(t_i) \implies t \in \subterms(f^n(..., t_i, ...))$.
\end{itemize}

We say that possibly infinite term $t \in T^\infty$ is rational\inRocq{is_rational_term} iff their set of subterms is finite~\cite{colmerauer1982prolog}. Now we can define the set of rational terms: $T^R = \{ t \in T^\infty \mid \subterms(t) \text{ finite}\}$.

For example, $f(f(f(...)))$ is rational term since it has a single subterm (itself) but $f_1(f_2(f_3(...)))$ where the ellipsis stands for the sequence of different constructor applications is not rational term. Obviously, any finite term is rational, thus $T \subset T^R$\inRocq{term_to_inf_rational}.
