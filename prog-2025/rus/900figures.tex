...
\begin{figure}[t]
	...
	\subfloat[Пример реального расположения элементов управления\label{form1-layout}]{
		\scalebox{0.5}{
			\begin{tikzpicture}
				\node (label) at (7.93, 2.9) {\emph{justify}};
				...
				\node[rectangle,draw] (combo) at (13.2, 0) {\includegraphics[scale=0.2]{../eng/combo_rus.png}};
				\begin{scope}
					\node[rectangle,dashed,draw,fit=(label)(combo)] (composite) (0, 0) {};
				\end{scope}
				\path[draw,<->] (label.east) -- node[above] {\emph{inset}} (combo.west);
				\path[draw,<->] (check.south) -- node[right] {\emph{inset}} (10.46,0.71);
				\path[draw,-] (7.3, 3.3) -- (7.3, 0);
 	\end{tikzpicture}}}
	\vskip5mm
