...
Для получения более строгих числовых результатов, мы также применили критерий Краскела-Уоллиса~\cite{kruskal1952use} и пост-хок критерий Данна~\cite{dunn1964multiple} с поправкой Бонферрони~\cite{bonferroni1936teoria} к полученным замерам. В результате для каждой трассы было определено, какой алгоритм с ней справляется статистически-значимо лучше в рамках каждого варианта реализации. Итоговые результаты анализа приведены в \autoref{tab:analyse} в качестве числа <<побед>> по каждому алгоритму в рамках одного варианта реализации.
