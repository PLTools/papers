...
This lattice designates which of the arguments are ``physically equal'' to their \commonpart. With it, the optimization is captured by the function $\commonpart' : T \times T \rightarrow (T \times \textsf{Side})^?$. While non-recursive cases are trivial, during performing recursive calls we are accumulating the infimum of the sides:
\begin{itemize}
    \item if it is $\bot$, we are required to return the accumulated term (as in the simple implementation) -- there is no way to preserve a ``physical equality'' to any of the arguments;
