...
%Then we present our approach to handle user-defined types by injecting them into the logic domain, and describe a convenient generic programming pattern, which can be used to implement the conversions from/to logic domain.
...
%We also consider a simple approach and a more elaborate and efficient tagless variant (see Section~\ref{sec:injection}).
...
%top-level primitives and addresses the problem of relational and functional code integration.
Затем представлены несколько примеров (раздел~~\ref{sec:examples}) написания реляционных программ, с помощью нашей библиотеки.
%in Section~\ref{sec:examples} we present a set of relational examples, written with the aid of our library.
...
%Section~\ref{sec:evaluation} contains the results of a performance evaluation and a comparison of our implementations with existing implementation for Scheme.
