...
Полноценная реализация алгоритма со всеми оптимизациями требует значительного количества кода\footnote{С примерами такой реализации можно ознакомиться по ссылке \url{https://github.com/ProgMiner/rational-unification}.}, поэтому в этом разделе описаны только её основные элементы.

Для простоты примеры кода приводятся на языке программирования \OCaml. Будем считать, что термы заданы следующим алгебраическим типом данных:
...
На практике при реализации \union стоит применять эвристику <<возраста переменных>>~\cite{mannila1986complexity}: по возможности связывать <<более свежие>> переменные (обладающие б\'{о}льшими номерами) с <<менее свежими>>. Обоснование данной эвристики описано в процитированной статье, но на практике она позволяет уменьшить глубину рекурсии \walk подобно тому, как это делает ранговая эвристика в структуре данных СНМ (системе непересекающихся множеств~\cite{galler1964improved}). Примеры реализации с использованием этой оптимизации приведены в репозитории с анализом производительности алгоритма.
...
Тогда можно реализовать обобщённую функцию $\commonpart' : T \times T \rightarrow (T \times \textsf{Side})^?$, которая вторым возвращаемым параметром в случае успеха дополнительно сообщает, какие из аргументов подходят на роль результата. Нерекурсивные вызовы в реализации очевидны, а при рекурсивных вызовах требуется накапливать точную нижнюю границу (т.н.г., $\inf$) <<сторон>>:
\begin{itemize}
    \item Если накопленная т.н.г.~--- $\bot$, функция вынуждена вернуть сконструированный терм, потому что никакой из аргументов не подходит на роль результата.
    \item Иначе~--- отбросить сконструированный терм и вернуть тот аргумент, на который указывает накопленная т.н.г.
\end{itemize}
...
Наконец, реализация \unify почти полностью повторяет псевдокод (\textsc{RationalUnify}):
\begin{minted}{OCaml}
