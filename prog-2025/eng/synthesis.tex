...
%We divided the search for layouts into several steps:

%\begin{itemize}
%\item finding a single maximal set of instances containing all applications of the guideline rules; this set can be (and, usually, is)
%  non-compatible;
%\item detecting all conflicts in this set of instances;
%\item non-deterministically eliminating each conflict obtaining sets of all maximal compatible layouts.
%\end{itemize}
...
%This verifier checks various types of conflicts and, being executed in the backwards direction,
%finds all these conflicts in the layout.

%For example, consider a conflict caused by a cyclic chain of layout instances

%\begin{lstlisting}[language=ocanren,basicstyle=\small]
%   $\term{prim}\,(X,\, X_1), \, \term{prim}\,(X_1,\, X_2), \, \ldots, \, \term{prim}\,(X_n,\, X)$
%\end{lstlisting}

%\noindent where $\term{prim}$ is any kind of layout primitives which cannot form loops. This set is non-compatible since there
%is no coordinate assignment which would respect all these constraints altogether since they require the control $X$ to be placed
%to the right and/or below itself.
...
%conflict can be simply eliminated by ignoring one or more layout primitives from this chain.
%In this process there is a certain subtlety: layout primitives can be ``locked'' together  depending on the actual guidelines.
%There is a whole
%search space in which we may delete one layout primitive by ``canceling'' the application of one rule or two others
%by canceling another.
%But, thanks to the nature of relational programming, all these possibilities are accounted for automatically.
%On this step we use relational programming techniques to emulate non-deterministic choice of rules to delete, the feature of running relations backwards is not needed here.
%Thus, for all detected conflicts we obtain all maximal compatible sets of layout instances without enumerating all
%non-maximal ones.
...
%\item $C\prg{.width}$, ~--- the width of GUI control $C$ (integer constant for real GUI controls and a \emph{variable} for virtual GUI controls);
%\item $C\prg{.height}$~--- the height of GUI control $C$ (integer constant for real GUI controls and a \emph{variable} for virtual GUI controls);
\item $C\prg{.x}$,  $C\prg{.y}$~--- the $X$- and $Y$-coordinates of a GUI control $C$ (a \emph{variable} for each GUI control);
%\item $C\prg{.x}$~--- the $X$-coordinate of GUI control $C$ (a \emph{variable} for each GUI control);
%\item $C\prg{.y}$~--- the $Y$-coordinate of GUI control $C$ (a \emph{variable} for each GUI control);
