...
\begin{itemize}
    \item if $\union(\xi, x, y) = (\xi', \bot)$ then $\llbracket \xi' \rrbracket$ is a minimal unifying extension of $\llbracket \xi \rrbracket$ for $x, y$ (mostly by \autoref{lemma:unbound-unifier});
    \item if $\union(\xi, x, y) = (\xi_1, (t_1, t_2))$ then $\llbracket \xi_2 \rrbracket$ is a minimal unifying extension of $\llbracket \xi \rrbracket$ for $x, y$ iff $\llbracket \xi_2 \rrbracket$ is a minimal unifying extension of $\xi_1$ for $t_1, t_2$.
\end{itemize}
...
We define ``cost'' of two terms $\cost : T \times T \rightarrow \N$ as a recursively defined function\inRocq{rational_unification_cost}:
\begin{equation*}
    \begin{array}{r@{{}={}}l}
        \cost(x, t_2) & \size(t_2) \\
        \cost(t_1, y) & \size(t_1) \\
        \cost(f^n(l_1, ..., l_n), g^m(r_1, ..., r_m)) & 1 + \sum_{i = 1}^{\min(n, m)} \cost(l_i, r_i) \\
    \end{array}
\end{equation*}

Actually, independently from the actual definition, we only need that:
\begin{itemize}
    \item $\cost(x, t) = \cost(t, x) = \size(t)$ --- obviously by the definition;
    \item if $\commonpart(t_1, t_2) = t$ then $\cost(t_1, t_2) + \size(t) = \size(t_1) + \size(t_2)$ \\
    \inRocq{rational_unification_cost_common_part}. In other words, that $\cost(t_1, t_2)$ is the sum of the $t_1, t_2$ non-variable sizes without the non-variable size of its' common part.
\end{itemize}
...
\begin{figure}
    ...
    \begin{subfigure}{.99\linewidth}
        \begin{prooftree}
            ...
            \noLine
            \UnaryInfC{$\size(\xi_2) \le \size(\xi_1) + \sum_{j = 1}^{i - 1} \cost(l_j, r_j)$}
...
\[\size_X(\xi, t_1, t_2) = (|\roots_X(\xi)|, \size(\xi) + \cost(t_1, t_2)).\]
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \begin{enumerate}
                ...
                \begin{itemize}
                    \item Since $|\roots_X(\xi_1)| = |\roots_X(\xi_2)|$, we only need to show that $\size(\xi_1) + \cost(x, t_y) = \size(\xi_2) + \cost(t_x, t_y)$.
                    \item By the definition of \cost, $\cost(x, t_y) = \size(t_y)$.
                    \item By the property of common part and \cost, $\cost(t_x, t_y) + \size(t) = \size(t_x) + \size(t_y)$.
                \end{itemize}
                \begin{equation*}
                    \begin{array}{r@{{}={}}l}
                        \size(\xi_1) + \underline{\cost(x, t_y)} + \size(t) & \size(\xi_1) + \underline{\size(t_y) + \size(t)} \\
                        & \underline{\size(\xi_1) + \size(t)} + \size(t_y) \\
                        & \size(\xi_2) + \underline{\size(t_x) + \size(t_y)} \\
                        & \size(\xi_2) + \cost(t_x, t_y) + \size(t).
                    \end{array}
                \end{equation*}
                \item Let's consider the possible recursive calls from $(\xi_2, t_x, t_y)$. Since $t_x, t_y \not\in \V$, it's only \nameref{rule:subterm-call}. We may see that any \nameref{rule:subterm-call} recursive call strictly reduces the terms \cost because of the top-level constructors so we may use IH.
            \end{enumerate}
        \end{itemize}
        \item If $t_1 \not\in \V$, the only allowed recursive calls are \nameref{rule:swap-var-term-call} and \nameref{rule:subterm-call}. The first one just swaps terms of the task and immediately turns into the previous case. The latter always strictly reduces the terms \cost because of the top-level constructors so we may use IH.
    \end{itemize}
\end{proof}
