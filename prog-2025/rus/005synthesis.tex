...
В данном разделе мы кратко опишем верификатор расположений на основе предписаний (англ. guideline), который получается
...
Например, рассмотрим следующее правило:\footnote{\textcolor{red}{Может для этой статьи выбрать другое правило?}}
\[
...
Максимальность проверяется другим образом: мы запускаем верификатор на конкретной структуре и
...
Затем мы отфильтруем не максимальные.

На этом первый шаг закончен, и мы получили полный неэффективный синтезатор.
У наивной реализации можно указать два недостатка.
...
Чтобы решить эту проблему мы реализовали так называемый \emph{гиперкуб}~--- структуру данных, которая напоминает битовую карту (англ. bitmap).
...
Чтобы это решить, будет стоит  максимальное несовместное расположение, а из него получать все совместные путём поиска конфликтов
...
Это является оптимизаций за счет того, что конфликтующих примитивов расположения меньше, чем не конфликтующих, и поэтому проще получить все и недерминированно выкинуть все конфликтующие, чем недетерминированно искать все не конфликтующие.
...
специальный верификатор, который по гиперкубу и набору экземпляров расположения, проверяет, что элементы набора содержатся в гиперкубе и действительно образуют рассматриваемый конфликт.\footnotetext{\textcolor{red}{Пётр, перепроверь это место, потому что по-моему в ворде как-то стрёмно написано.} }
Запустившись в обратном направлении на конкретном гиперкубе верификатор вернёт набор экземпляров расположения, который образует конфликт соответствующего вида. Запуск в обратном направлении дизъюнкции верификаторов даст нам все конфликтующие экземпляры.
...
Так мы получим максимальное совместное расположение, когда не останется ни одного конфликта.
Тут есть тонкость, осложняющая реализацию:
удаляя экземпляры мы можем получить расположение, которые более не подтверждается предписанием, так как некоторые части расположения могут появляться и исчезать только одновременно.
