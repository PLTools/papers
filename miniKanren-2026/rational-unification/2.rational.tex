...
Formally, we consider countable infinite sets $x, y, z, \dots \in \V$ of variables and $f, g, h, \dots \in \C$ of constructors. The latter is equipped with the function $\arity : \C \rightarrow \N$ which determines the arities of constructors and the notation $f^n \in \C$ where $f \in C$ and $n = \arity(f)$.
...
For convenience, we will use the notation $A^? = \{ \bot \} \cup A$ for some set $A$ and call it ``optional $A$''. Also, we use the notation $\pi_i$ for the $i$-th projection of a tuple; for example, $\pi_1 : A \times B \rightarrow A$.
...
\begin{algorithmic}
    \Function{ApplyEqSys}{$\xi \in \Xi$, $t \in T^\infty$}
        ...
        \If{$x' \gets t \in \V$}
            \State \Return $x'$
        \EndIf
        \If{$f^n(\dots, t_i, \dots) \gets t$}
            \State \Return $f^n(\dots, \Call{ApplyEqSys}{\xi, t_i}, \dots)$
        \EndIf
    \EndFunction
\end{algorithmic}
...
\begin{equation*}
    \begin{split}
        \commonpart(x, \_) &= x \\
        \commonpart(\_, y) &= y \\
        \commonpart(f^n(\dots, l_i, \dots), f^n(\dots, r_i, \dots)) &= f^n(\dots, \commonpart(l_i, r_i), \dots) \\
    \end{split}
\end{equation*}
...
\begin{itemize}
    \item Correctness: if the algorithm terminated with the non-$\bot$ result, the result is a minimal unifying extension of a source equation system for the given terms.
    \item Completeness: if the algorithm terminated with $\bot$ result, there is no unifying extension of a source equation system for the given terms.
