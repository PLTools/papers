...
Вычисление конструкции  \lstinline[language=ocanren,basicstyle=\small]|run$^*$ {appendo x y z}| возвращает \emph{ленивый поток ответов}, который содержит все подстановки этих трёх переменных, такие, что отношение $\{(x,y,z)\in\mathbb{N}^3 \mid x +\kern-1.3ex+\kern0.8ex y=z\}$ верно.
